...
// Answer: Function, prototypes tell the compiler about the functions name, type return, and parameters
...
void  word_print(char *str){
   ...
   // int last_char_idx;  //index of last char - strlen(str)-1;
    ...
    for(int i = 0; str[i] != '\0'; i++){
        ...
        if(word_start && (str[i]== SPACE_CHAR || str[i + 1] == '\0')) {
            printf(" (%d)\n", wlen);
            word_start = false;
        }
    }
...
int main(int argc, char *argv[]){
    ...
    switch (opt){
        case 'c':
            int wc = count_words(input_string); // calling word count
        ...
        case 'r':
            ...
            reverse_string(input_string); // reverses the string
        ...
        //          It ensures that invalid inputs are handled. If the options are other than
        // -h, -c, -r, -w, the program displays the usage instructions and exits with an error message.
        default:
            usage(argv[0]);
    ...
    //The break statement prevents the fall-through in switch cases. Without it, execution continues into the next caser, causing unknown behaviour.
}
